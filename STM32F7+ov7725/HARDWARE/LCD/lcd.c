...
{ 	 
	...
	FMC_Bank1->BTCR[1]|=0XF<<0; 	//地址建立时间(ADDSET)为15个HCLK 1/216M=4.6ns*15=69ns	
	//因为液晶驱动IC的读数据的时候，速度不能太快,尤其是个别奇葩芯片。
	FMC_Bank1->BTCR[1]|=80<<8;  	//数据保存时间(DATAST)为80个HCLK=4.6*80=368ns
	//写时序控制寄存器  
	FMC_Bank1E->BWTR[0]|=0<<28; 	//模式A 	 							    
	FMC_Bank1E->BWTR[0]|=15<<0;		//地址建立时间(ADDSET)为15个HCLK=69ns
	//15个HCLK（HCLK=216M）,某些液晶驱动IC的写信号脉宽，最少也得50ns。  	 
	FMC_Bank1E->BWTR[0]|=15<<8; 	//数据保存时间(DATAST)为4.6ns*15个HCLK=69ns
	//使能BANK1,区域1
	...
	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963)//如果是这几个IC,则设置WR时序为最快
	{
		...
		FMC_Bank1E->BWTR[0]|=4<<0;	 	//地址建立时间(ADDSET)为4个HCLK =18.4ns  	 
		FMC_Bank1E->BWTR[0]|=4<<8; 		//数据保存时间(DATAST)为4.6ns*4个HCLK=18.4ns
	} 
